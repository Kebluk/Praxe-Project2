...
On the KL25, the UART0 peripheral is a low-power UART that uses an Oversampling Ratio (OSR). By default, the OSR is set to 15 (which means an oversampling rate of $15 + 1 = 16$).
The correct formula for the baud rate selection register (SBR) is:


$$\text{Baudrate} = \frac{\text{UART0\_Clock}}{\text{SBR} \times (\text{OSR} + 1)}$$
...
The Problem: On the FRDM-KL25Z board, the onboard RGB LED is wired in an active-low configuration. Writing a logic high ($3.3\text{V}$ / high PWM duty cycle) turns the LED OFF, while writing a logic low ($0\text{V}$ / zero PWM duty cycle) turns it ON.
